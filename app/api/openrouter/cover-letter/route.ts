...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const applicantInfo = await getApplicantInfo(resume, token);
    let prompt = ""
    let systemPrompt = ""
    let llmModel = ""
...
1. Header:
   - Candidate's name: ${applicantInfo.name}
   - Contact information: ${applicantInfo.email}${applicantInfo.phone !== '[Your Phone]' ? ` | ${applicantInfo.phone}` : ''}${applicantInfo.address !== '[Your Address]' ? ` | ${applicantInfo.address}` : ''}
   - Date
